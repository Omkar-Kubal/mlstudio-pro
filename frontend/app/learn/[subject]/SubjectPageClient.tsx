...
import { getContentMapping, getLessonId } from "@/adapters/content-mapping";
...
export default function SubjectPageClient({ subjectSlug }: { subjectSlug: string }) {
    const { user } = useAuth();
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                // Get modules for this subject from local adapter
                const filteredModules = allModules.filter(m => m.subjectSlug === subjectSlug);
                setModules(filteredModules);

                // Fetch real user progress from backend
                const stats = await apiFetch<ProgressStats>("/curriculum/progress");
            ...
            <main className="flex-grow pt-24 pb-20 px-6 sm:px-8 lg:px-12 max-w-7xl mx-auto w-full">
                ...
                <section className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-12">
                    ...
                    <div className="glass-panel p-5 rounded-lg flex flex-col shadow-silver-glow">
                        ...
                        <div className="mt-2 flex items-baseline gap-2">
                            <span className="text-3xl font-bold text-white">{totalCompletedTopics}</span>
                            <span className="text-sm text-muted">/ {totalTopicsInSubject} topics</span>
                        </div>
                    </div>
                    <div className="glass-panel p-5 rounded-lg flex flex-col shadow-silver-glow">
                        <span className="text-xs text-muted uppercase font-semibold tracking-wider">Current Streak</span>
                        <div className="mt-2 flex items-baseline gap-2">
                            <span className="text-3xl font-bold text-white">{progressStats?.streak || 0}</span>
                            <span className="text-sm text-muted">days</span>
                        </div>
                    </div>
                    <div className="glass-panel p-5 rounded-lg flex flex-col shadow-silver-glow">
                        <span className="text-xs text-muted uppercase font-semibold tracking-wider">Total XP</span>
                        <div className="mt-2 flex items-baseline gap-2">
                            <span className="text-3xl font-bold text-white">{progressStats?.xp || 0}</span>
                            <span className="text-sm text-muted">pts</span>
                        </div>
                    </div>
                    <Link
                        href={`/learn/${subjectSlug}/resources`}
                    ...
                    >
                        ...
                        <div className="mt-2 flex items-center justify-between">
                            <span className="text-xl font-bold text-white">Subject Docs</span>
                ...
                <section className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                    {modules.map((module, index) => {
                        ...
                        // Check if module is unlocked based on backend sequential logic
                        const mapping = getContentMapping(subjectSlug, module.slug);
                        const lessonId = mapping ? `s${mapping.subjectIndex}m${mapping.moduleIndex}` : '';
                        const isUnlocked = progressStats?.unlocked_modules.includes(lessonId) || index === 0;
